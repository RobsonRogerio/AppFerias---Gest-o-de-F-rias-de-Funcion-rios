...
def verifica_e_adiciona_ferias(data_inicio, data_final):
    usuario = st.session_state['usuario']
    total_dias = (
    ...
    if total_dias < 5:
        ...
        st.error(f'Usuário solicitou {total_dias} dias, porém tem direito a {dias_solicitar}')
    else:
        usuario.adiciona_ferias(data_inicio, data_final)
    limpar_datas()